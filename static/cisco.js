...
authenticate.addEventListener('click', function () {
  ...
  fetch('/getSwitchData_Cisco', { method: 'POST', body: JSON.stringify({ switchIP: hostField.value, switchUsername: username.value, switchGeneralPassword: generalPassword.value, switchSecretPassword: secretPassword.value }) })
    .then(res => res.json())
    .then(res => {
      console.log(res);
...
portSettingsUpdateButton.addEventListener('click', function () {
  ...
  fetch('/updatePort_Cisco', { method: 'POST', body: JSON.stringify({ switchIP: hostField.value, switchUsername: username.value, switchGeneralPassword: generalPassword.value, switchSecretPassword: secretPassword.value, command: sendCommand }) })
    .then(res => res.json())
    .then(res => {
      console.log(res)
      location.href = 'http://127.0.0.1:5000/cisco?host=' + hostField.value + '&username=' + username.value + '&generalPassword=' + generalPassword.value + '&secretPassword=' + secretPassword.value;
    });
...
customCommandSend.addEventListener('click', function () {
  ...
  fetch('/sendCustomCommand_Cisco', { method: 'POST', body: JSON.stringify({ switchIP: hostField.value, switchUsername: username.value, switchGeneralPassword: generalPassword.value, switchSecretPassword: secretPassword.value, customCommand: customCommandInput.value }) })
    .then(res => res.json())
    .then(res => {
